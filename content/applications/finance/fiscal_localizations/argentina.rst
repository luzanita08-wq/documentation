...
.. list-table::
   ...
   * - :guilabel:`Argentina - Accounting`
     ...
     - Default :ref:`fiscal localization package <fiscal_localizations/packages>`, which represents
       the minimal configuration to operate in Argentina under the :abbr:`ARCA (Agencia de
       Recaudación y Control Aduanero)` regulations and guidelines.
...
In Accounting, three distinct :doc:`charts of accounts
<../accounting/get_started/chart_of_accounts>` packages are available when :ref:`configuring the
Argentinian fiscal localization <fiscal_localizations/packages>`. Selection depends on the company’s
|ARCA| responsibility type and reflects the different levels of fiscal complexity required:
...
The information required for the :guilabel:`Document Types` is included by default, so the user must
not fill anything in the :guilabel:`Document Types` view.

.. note::
   The list displayed is not exhaustive and does not include all available document types.
...
For sales and purchase journals, the :guilabel:`Use Documents` option can be activated, enabling a
list of :guilabel:`Document Types` to be linked to invoices and vendor bills. For more details on
invoices, please refer to section :ref:`2.3 document types
<localizations/argentina/document-types>`.
...
- :guilabel:`wsfev1: Electronic Invoice`: Is the most common service, used to generate invoices for
...
- :guilabel:`wsfexv1: Export Voucher`: Is used to generate invoices for international customers and
  transactions involving export processes; the document type related is type "E".

Here are some useful fields to know when working with web services:

- :guilabel:`ARCA POS Number`: Is the number configured in |ARCA| to identify the operations
  related to this ARCA POS.
- :guilabel:`ARCA POS Address`: Is the field related to the commercial address registered for the
  ...
  location. This location will appear on the invoice report.
...
:menuselection:`Accounting --> Reporting`, in the :guilabel:`Argentinean Statements`
section.
...
.. note::
   Click the :icon:`fa-cog` menu and select :guilabel:`VAT Book (ZIP)` or :guilabel:`VAT Simple
   Report (ZIP)` to generate the VAT Book or VAT Simple report ZIP file, respectively.
...
not sent to the ARCA. To access it, go to :menuselection:`Accounting --> Reporting --> VAT Summary`.
...
This pivot table allows you to validate the gross income for each jurisdiction. It serves as an
affidavit for the corresponding taxes due, but is not submitted to the ARCA. To access it, go to
...
This pivot table allows you to validate the gross purchases for each jurisdiction. It serves as an
affidavit for the corresponding taxes due, but is not submitted to the ARCA. To access it, go to
...
Once the :ref:`localization modules are installed <localizations/argentina/modules>`, the first step
is to set up the :ref:`company's record <general/companies/configuration>`. In addition to the basic
information, make sure to complete the Ecuador-specific field, :guilabel:`ARCA Responsibility Type`,
to define the company's fiscal obligation and structure.
...
As part of the Argentinian localization, document types defined by |ARCA| are available in the
:doc:`contact form <../../essentials/contacts>`. Information is essential for most transactions.
Six :guilabel:`Identification Types` are available by default, as well as 32 inactive types.
...
Certificate` if you don't already have one:
...
#. Obtain Certificate (ARCA): Access the |ARCA| portal and follow the instructions described in
   ...
   a certificate.
#. Certificate and Private Key (Odoo): To upload the certificate to Odoo, click the
   :icon:`oi-arrow-right` :guilabel:`(right arrow)` icon next to the :guilabel:`Certificate` field
   and select the corresponding file. Then add the existing private key that was created after
   clicking :guilabel:`Generate Certificate`.

.. tip::
   - When a created certificate expires, click :guilabel:`Generate Renewal Request` to generate a
     new certificate.
   - In case you must configure the Homologation Certificate, please refer to the |ARCA| official
     documentation: `Homologation Certificate
     <http://www.arca.gob.ar/ws/documentacion/certificados.asp>`_. Furthermore, Odoo allows the user
...
Once the invoice is posted, the information related to |ARCA| result appears on the
invoice, and the :guilabel:`Result` field indicates if the invoice has been :guilabel:`Aceptado en
ARCA` or :guilabel:`Aceptado con Observaciones`.
...
- :guilabel:`A. Tax excluded`: In this case the taxed amount must be clearly identified in the
  report. This condition applies when the customer has the following :guilabel:`ARCA Responsibility
  type` of :guilabel:`Responsable Inscripto`.
...
starting and ending dates. To do so, fill in the :guilabel:`Service Date` field in the
...
benefit from the Electronic Tax Bonds granted by the Ministry of Economy.
...
For these transactions, consider the following requirements:

- Specific document types (201, 202, 206, etc).
- The emitter should be eligible for the |ARCA| to MiPyME transactions.
- The amount should be larger than 100,000 ARS.
- A bank account type CBU must be associated with the emisor; otherwise, the invoice cannot be
...
The :guilabel:`PDF Report` for electronic invoices validated by |ARCA| includes a barcode at the
bottom of the format, which represents the CAE number. The expiration date is also displayed, as it
...
.. tip::
   ...
   :guilabel:`Number` and use this reference for any potential issues on the sequence
...
automatically. However, if the format is invalid, a user error will be displayed indicating the
expected format.
...
document, an automatic validation can be set in :menuselection:`Accounting --> Configuration -->
Settings`. Scroll down to the :guilabel:`Argentinean Localization` section and consider the
following levels in the :guilabel:`Verify Vendor Bills validity in ARCA` section:

- :guilabel:`Not available:` the verification is not complete (this is the default value).
- :guilabel:`Available:` the verification is complete. If the number is not valid, it only displays
  a warning but still allows the vendor bill to be posted.
- :guilabel:`Required:` the verification is complete, and it does not allow the user to post the
  vendor bill if the document number is not valid.
...
Once the vendor validation settings are enabled, click :guilabel:`Verify on ARCA` in the
...
While Odoo already creates most of the required withholdings the :guilabel:`Taxes` menu, in several
cases it is necessary to apply or modify configurations to correctly calculate the withholding
amount on vendor payments. The following withholding types are available:
...
In this particular case, a percentage must not be set. Instead, this withholding is calculated based
on the value of the :guilabel:`Scale` field.
...
.. note::
   Earnings scales are cumulative, Odoo keeps track of the different records created for a bill and
   automatically calculates the correct withholding amount.
...
The recommendation, in this case, is always to duplicate and apply the different configurations to
...
.. important::
   The amount of the debt to be canceled equals the total payment amount. However, Odoo
...
In the payment registration modal, select the bank journal from which the payment is to be made and
set the :guilabel:`Check Cash-In Date` and :guilabel:`Amount`.
...
reconciliation between vendor bills and bank statements, leaving the check marked as **cancelled**.
...
manually add the :guilabel:`Check Issuer Vat`, but it is automatically filled with the customer's
