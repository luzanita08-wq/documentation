...
To access the reporting menu, navigate to :menuselection:`Accounting --> Reporting --> Tax Report`,
click :icon:`fa-book` :guilabel:`Report`, and select :guilabel:`2306 & 2307`.

Based on the period filtered, Odoo displays the transactions subject to each EWHT (Expanded Withholding
Tax) and FWHT (Final Withholding Tax) grouped by ATC code per partner level.

To export the certificates into PDF:

#. Make sure to filter by the appropriate **Month** or **Quarter** using the calendar filter.
#. By default, Odoo exports the certificates for all the partners applicable in the report. To
   specifically choose a partner, filter using the :guilabel:`Partners` filter.
#. Click the :guilabel:`PDF` button on the top-left corner of the screen to export the BIR-compliant
   PDF certificate.

.. note::
   When exporting for a single partner, Odoo downloads an individual PDF file. If the export contains
   multiple certificates for different partners, Odoo compiles them into a single `.ZIP` file.
