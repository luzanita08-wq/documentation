...
.. tip::
   ...
   .. note::
      ...
      assigned to a category. Otherwise, it will not be displayed or available for selection.
...
- the :doc:`available
  ...
  and :doc:`forecasted
  </applications/inventory_and_mrp/inventory/warehouses_storage/reporting/forecast>` stock,
  including :doc:`warehouse
...
You can also access and edit product information of a product that has already been :ref:`added to
the cart <pos/use/sell>` by selecting the product, clicking the :icon:`fa-ellipsis-v`
...
- If applicable, configure :doc:`Urpan Piper <restaurant/online_food_delivery>` and/or decide if the
...
:guilabel:`Product Tags` page, click :guilabel:`New` or select an existing
tag.
...
- Optionally, choose a :guilabel:`Color` or add an image. If both are configured, the image replaces
  the tag's color.
- In the :guilabel:`Point of sale` section, write a description to be shown in self-ordering mode.
...
.. note::
   If the **eCommerce** app is installed, tags are used in both apps, and need to be added in the
   :guilabel:`Ecommerce shop` section of the :guilabel:`Sales` tab.
...
#. On the category form:
   ...
   - If :doc:`Online food delivery platforms
     </applications/sales/point_of_sale/restaurant/online_food_delivery>` are configured in your
...
To limit categories displayed in the POS register, go to the :ref:`POS settings <pos/use/settings>`,
navigate to the :guilabel:`Product & PoS categories` section, and select the relevant categories in
the :guilabel:`Restrict Categories` field.
...
.. example::
    ...
    .. tip::
       To change the selection after :ref:`adding a product combo to the cart <pos/use/sell>`, click
       and hold the combo item in the cart to reopen the selection popover.
...
.. note::
   The total price of the product combo displayed in the :ref:`POS register
...
To track stock for POS products, the :ref:`product type
<inventory/product_management/goods-or-services>` must be set to :guilabel:`Goods` or
:guilabel:`Combo`, and the :ref:`track inventory option
...
.. tip::
   ...
   can also be configured for products tracked by lot or serial number.
