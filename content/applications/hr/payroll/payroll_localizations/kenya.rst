...
.. list-table::
   ...
   * - :guilabel:`Documents - Kenyan Payroll`
     ...
     - Employee Tax Deduction Card forms are automatically integrated to the Document app.
...
- :guilabel:`Mortgage interest`: Populate the mployee's mortgage interest rate, if applicable.
...
- :guilabel:`SHIF Number`: Enter the employee's 7 or 8 digit  |SHIF| number. This field si required
...
- :guilabel:`Insurance`: Enter the monthhly :guilabel:`Voluntary Medical insurance`, :guilabel:`Life
...
with another employer. When checked, the *Insurance* section is removed from the *Payroll* tab,
as insurance is managed by the primary employer.
...
The 8Salary Inputs* tab of a pyslip includes additional benefits and deductions that may vary per
...
.. note::
   ...
   by the employee's the primary employer.
...
--> Payslip Lines`. Next, click inot the search bar and click the :guilabel:`Date` drop-down menu,
reveaing various months and quarters. Click the desired month, and the report displays all payslpis
for that month.
...
Each individual employee receives a *tax deduction card*, which contains all their various
earnings.

The various *tax deductoin cards* together, is what the P9 report is comprised of.
...
To view the employees and generate the individual cards, click th e :icon:`fa-users`
...
of the employee list. All successfully generated rpeorts have a :guilabel:`State` of
...
.. image:: kenya/p9.png
   :alt: The list of employees with a generated P9 report.
...
dates by default, but can be modifued using the drop-down menu.
...
Any individual payslpi can be removed from the report by clicking the :icon:`fa-trash-o`
:guilabel:`(Delete)` icon at the end of the row.

Click the :guilabel:`Export XLSX` button, and the report is generated.

.. image:: kenya/shif.png
   :alt: The SHIF (NHIF) Report pop-up iwndow configured for June of 2026.
