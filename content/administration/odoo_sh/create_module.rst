...
- :file:`__manifest__.py` the manifest of your module, including its title, description and data
  ...
  Uncomment the access control list data file:
...
#. These changes alter the database structure by adding a column to a table and modifying a view.
   To be applied to an existing database, such as your production database, these changes require
   the module to be updated. If you would like the update to be performed automatically by the
   Odoo.sh platform when you push your changes, increase the module's version in its manifest by
   opening :file:`__manifest__.py` and replacing:
...
#. Next, :ref:`push on the development branch <odoo-sh/module/push-development>`.
...
#. You can now use the library in your module, for instance to remove accents from characters in the
   name field of your model. To do so, open the :file:`models/models.py` file and before:
...
#. Adding a Python dependency requires increasing the module version for the platform to install it.
   To do so, edit the :ref:`module's manifest <odoo-sh/module/add/manifest>`
   :file:`__manifest__.py`.

#. Then, :ref:`push on the development branch <odoo-sh/module/push-development>`.
